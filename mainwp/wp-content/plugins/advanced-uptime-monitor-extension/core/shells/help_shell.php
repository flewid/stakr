...
class HelpShell extends MvcShell {
	...
	public function shell( $args ) {
		...
		if ( ! empty( $method ) && ! empty( $shell->methods[ $method ] ) ) {
			$table->add_row( array( Console_Color::convert( '%9'.$method.'%n' ), $shell->methods[ $method ] ) );
		} else {
			foreach ( $shell->methods as $method => $doc ) {
				$table->add_row( array( Console_Color::convert( '%9'.$method.'%n' ), $doc ) );
				$table->add_separator();
			}
		}
		$this->out( $table->get_table() );
